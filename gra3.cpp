...
queue<pair<int,int> > dz[2]; //pozycja, platforma
...
int main()
{
	...
	for(int cc=1; cc<=n; cc++)
	{
		...
		for(int i=1; i<=k; i++)
		{
			...
			while(!dz[0].empty())
			{
				if(dz[0].front().first<h)
				{
					G[cv].push_back(make_pair(dz[0].front().second,0));
					//st[dz[cc-1].front().second]++;
					G[dz[0].front().second+1].push_back(make_pair(cv,1));
					//st[cv]++;
					dz[0].pop();
				}
				else
